...
describe("resolveXVideoFromApi", () => {
  test("resolves a matching video from the signed-in GraphQL response before guest activation", async () => {
    ...
    expect(requestHeaders.map((request) => request.url)).not.toContain(
      "https://api.x.com/1.1/guest/activate.json",
    );
  });

  test("falls back to twitter API host when x API guest activation is rejected", async () => {
    const guestRequests: string[] = [];
    const fetchMock = vi.fn<typeof fetch>(async (input, init) => {
      const url = String(input);

      if (url === "https://abs.twimg.com/responsive-web/client-web/main.456.js") {
        return new Response(
          'authorization:"Bearer BBBBBBBBBBBBBBBBBBBBBB",e.exports={queryId:"def456",' +
            'operationName:"TweetResultByRestId",operationType:"query",' +
            'metadata:{featureSwitches:["feature_a"],fieldToggles:["field_a"]}}',
        );
      }
      ...
      if (url === "https://api.x.com/1.1/guest/activate.json") {
        guestRequests.push(url);
        return new Response("", { status: 403 });
      }

      if (url === "https://api.twitter.com/1.1/guest/activate.json") {
        guestRequests.push(url);
        return Response.json({ guest_token: "fallback-guest-token" });
      }

      if (url.startsWith("https://x.com/i/api/graphql/def456/TweetResultByRestId?")) {
        const headers = init?.headers as Record<string, string> | undefined;

        if (headers?.["x-guest-token"] !== "fallback-guest-token") {
          return new Response("", { status: 403 });
        }
      ...
      resolveXVideoFromApi(
        {
          ...
          mediaId: "2072046031202562048",
          pageUrl: "https://x.com/camisa8BRASIL/status/2072068448268775859",
          mainBundleUrls: ["https://abs.twimg.com/responsive-web/client-web/main.456.js"],
        },
        fetchMock,
      ),
